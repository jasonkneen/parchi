...
import { shouldEnableAnthropicThinking } from './agent-loop-context.js';
import type { ModelConfig } from './agent-loop-model.js';
...
export function assemblePreparedAgentLoop(params: {
  ...
  context: import('./agent-loop-shared.js').AgentLoopContext;
  matchedSkillsResult: import('./agent-loop-shared.js').MatchedSkill[];
  preparedHistory: {
...
function buildModelFromProfile(profile: AgentProfile) {
  const { resolveLanguageModel } = require('../../ai/sdk-client.js');
